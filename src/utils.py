...
def LoadJson(path):
    try:
        ...
        logger.error(f"Error: Unable to parse {path}. Will use default configuration.")
        return {}
    except Exception as e:
        logger.error(f"Error: An error occurred while loading configuration: {str(e).encode('utf-8', errors='replace').decode('utf-8')}. Will use default configuration.")
        return {}
def LoadImage(path):
    try:
        # 尝试读取图片
        ...
        logger.error(f"Failed to load image: {str(e).encode('utf-8', errors='replace').decode('utf-8')}")
        return None
    return img
############################################
def sanitize_message(message):
    """Sanitize message to avoid encoding issues on Windows console"""
    try:
        ...
        # If all else fails, return a safe generic message
        return "Error message contained unsupported characters"
...
def SaveConfigToFile(config_data):
    try:
        ...
        logger.error(f"Error occurred while saving configuration: {str(e).encode('utf-8', errors='replace').decode('utf-8')}")
        return False
...
def ShowChangesLogWindow():
    ...
    try:
        ...
        text_area.configure(state='normal')
        text_area.insert(tk.INSERT, f"Error: {CHANGES_LOG} file not found")
        text_area.configure(state='disabled')
    
    except Exception as e:
        text_area.configure(state='normal')
        text_area.insert(tk.INSERT, f"Error reading file: {str(e)}")
        text_area.configure(state='disabled')
###########################################
...
def BuildQuestReflection():
    try:
        ...
        raise KeyError(f"'questName' attribute does not exist: {e}.")
...
def LoadTemplateImage(shortPathOfTarget):
    logger.debug(f"Loading {shortPathOfTarget}")
