        ...
        try:
            ...
            match command:
                case 'start_quest':
                    ...
                    logger.info(f'Starting task \"{self.quest_setting._FARM_TYPE+self.quest_setting._FARM_LVL}(Extra setting={self.quest_setting._FARM_EXTRA})\"...')
                ...
                case 'turn_to_7000G':
                    logger.info('Starting money request...')
                ...
                case 'error':
                    ...
                    if hasattr(self, 'progress_window') and self.progress_window.winfo_exists():
                        self.progress_window.destroy()
                    # Skip console logging entirely for update errors to prevent encoding crashes
                    # The error is already logged to file by auto_updater.py
                    # Show user-friendly message box
                    try:
                        if "Update failed:" in value:
                            display_msg = "Update failed. Please check the log files for details."
                        elif "Failed to check for updates:" in value:
                            display_msg = "Failed to check for updates. Please check your internet connection."
                        else:
                            display_msg = "An error occurred. Please check the log files for details."
                        messagebox.showerror("Error", display_msg, parent=self.main_window)
                    except:
                        messagebox.showerror("Error", "An error occurred during the operation.", parent=self.main_window)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='WvDAS command line arguments')
    
    # Add -headless flag parameter
    parser.add_argument(
        '-headless', 
        '--headless', 
        action='store_true',  # Mark as True when parameter is detected
